...
import {
	...
	resolve_intl,
	resolve_options,
} from '../../src/internals/resolve-options';
...
const { intl, ...rest } = DEFAULT_OPTIONS;
const FLATTEN_DEFAULT_OPTIONS = flatten(rest);
const FLATTEN_DEFAULT_INTL = flatten(intl);
...
describe('resolve_intl', () => {
	test('can specify intl.copy.default', () => {
		const value = 'Custom Copy';

		// as object
		let options = resolve_intl({}, { copy: { default: value } });
		expect(flatten(options)).toEqual({ ...FLATTEN_DEFAULT_INTL, 'copy.default': value });

		// as function
		options = resolve_intl({}, () => ({ copy: { default: value } }));
		expect(flatten(options)).toEqual({ ...FLATTEN_DEFAULT_INTL, 'copy.default': value });
	});
	...
	test('can specify intl.copy.copied', () => {
		...
		// as object
		let options = resolve_intl({}, { copy: { copied: value } });
		expect(flatten(options)).toEqual({ ...FLATTEN_DEFAULT_INTL, 'copy.copied': value });

		// as function
		options = resolve_intl({}, () => ({ copy: { copied: value } }));
		expect(flatten(options)).toEqual({ ...FLATTEN_DEFAULT_INTL, 'copy.copied': value });
	});
	...
	test('can specify intl.fullscreen.open', () => {
		...
		// as object
		let options = resolve_intl({}, { fullscreen: { open: value } });
		expect(flatten(options)).toEqual({ ...FLATTEN_DEFAULT_INTL, 'fullscreen.open': value });

		// as function
		options = resolve_intl({}, () => ({ fullscreen: { open: value } }));
		expect(flatten(options)).toEqual({ ...FLATTEN_DEFAULT_INTL, 'fullscreen.open': value });
	});
	...
	test('can specify intl.fullscreen.exit', () => {
		...
		// as object
		let options = resolve_intl({}, { fullscreen: { exit: value } });
		expect(flatten(options)).toEqual({ ...FLATTEN_DEFAULT_INTL, 'fullscreen.exit': value });

		// as function
		options = resolve_intl({}, () => ({ fullscreen: { exit: value } }));
		expect(flatten(options)).toEqual({ ...FLATTEN_DEFAULT_INTL, 'fullscreen.exit': value });
	});
	...
	test('can specify intl.collapse', () => {
		...
		// as object
		let options = resolve_intl({}, { collapse: value });
		expect(flatten(options)).toEqual({ ...FLATTEN_DEFAULT_INTL, collapse: value });

		// as function
		options = resolve_intl({}, () => ({ collapse: value }));
		expect(flatten(options)).toEqual({ ...FLATTEN_DEFAULT_INTL, collapse: value });
	});
});
